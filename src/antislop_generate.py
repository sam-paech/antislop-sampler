# File: ai/antislop-sampler/src/antislop_generate.py
...
# ADDED: import the custom LogitsProcessor for banning tokens
from src.logits_processor_slop import SlopTokenBanLogitsProcessor
# /ADDED
...
class AntiSlopSampler:
    def __init__(
        ...
        # ADDED
        ban_slop_first_tokens: bool = False
        # /ADDED
    ):
        self.model = model
        ...
        # ADDED: store a set of tokens that should be banned, if requested
        self.ban_slop_first_tokens = ban_slop_first_tokens
        if self.ban_slop_first_tokens:
            self.slop_banned_tokens = set()
            for phrase, tokenset in starting_tokens_lookup.items():
                self.slop_banned_tokens.update(tokenset)
        else:
            self.slop_banned_tokens = set()
        # /ADDED
    ...
    def _generate_streaming(self, current_input_ids, new_toks_to_generate, temperature, min_p, top_k, top_p, pad_token_id, stopping_criteria_args):
        ...
        # ADDED: build our LogitsProcessorList
        logits_processor_list = LogitsProcessorList()
        #print(len(self.slop_banned_tokens))
        if self.ban_slop_first_tokens and len(self.slop_banned_tokens) > 0:
            
            logits_processor_list.append(SlopTokenBanLogitsProcessor(self.slop_banned_tokens))
        # /ADDED
    ...
    def _filter_probs(self, probs: torch.FloatTensor, top_k: int, top_p: float, min_p: float) -> torch.FloatTensor:
        # Original code for re-sampling or re-ranking if needed.
        # Called only when regenerating from the probs_cache, not every step.
        probs = probs.clone()
        ...
        # We do NOT ban tokens here, because that is handled by the SlopTokenBanLogitsProcessor
        # at each step during generation.
...
def chat_antislop(
    ...
    Args:
        ...
        ban_slop_first_tokens (bool, optional): If True, ban the first tokens from slop phrases using a LogitsProcessor.
    ...
    return generate_antislop(
        ...
        # ADDED
        ban_slop_first_tokens=ban_slop_first_tokens
        # /ADDED
    )
        ...
        if streaming and stream_smoothing:
            ...
            while True:
                ...
                while len(generated_sequence) - last_released_position > backtracking_buffer_size:
                    while True:
                        try:
                            gen_seq2 = sampler.sequence_queue.get(timeout=0.001)
                            if len(gen_seq2) <= len(token_times):
                                token_times = token_times[:len(gen_seq2)-1]
                            generated_sequence = gen_seq2
                        except queue.Empty:
                            break
                    if sampler.generation_complete.is_set():
                        break
